...
//		new RegisterRequestValidator().validate(regReq, errors);
	...
	/* 커맨드 객체에 해당하는 파라미터(RegisterRequest)에 @Valid를 붙이면,글로벌 범위 Validator
	 * 은 해당 타입 검증 가능여부를 확인, 그 수행하면 결과를 Errors에 저장.
	 * 이는 메서드 수행 전에 확인하므로, 굳이 handleStep3()은 RegisterRequest 객체를 
	 * 검증하는 코드의 작성 필요 없이, 파라미터로 전달받은 Errors를 이용해 검증에러의 존재를 확인하면 됨.
	 * 
	 */
	
	/* 컨트로럴 범위 Validator의 설정은 InitBinder
	 * 현재 handleStep3()은 객체 파라미터에 Valid지정을 통해, Validator객체의
	 * validate메서드를 호출하지 않음.
	 * 어떤 Validator가 커맨드객체를 검증할지, initBinder가 결정.
	 * 즉, 현재 검증이 필요한 커맨드객체는 RegisterRequest이므로, setValidator은 이를 지원하는
	 * RegisterRequestValidator를 검증용으로 set한다.
	 * 
	 * 이는 handleStep1,2,3() 모두 실행시 적용되며, 아래 메서드는 WebDataBinder가 갖고있는
	 * validator을 목록에서 삭제 후, 파라미터로 전달받은 Validator을 목록에 추가.
	 * 즉, setValidator()을 사용시, 글로벌범위 Validator대신, set한 것을 사용함.
	 * 
	 * 이 상황에서 addValidator()을 실행시, 기존 Validator목록에 새것을 추가하는 것이므로,
	 * set과는 달리 우선 적용될 글로벌범위를 먼저한후 add를 씀.
	 */
	@InitBinder
